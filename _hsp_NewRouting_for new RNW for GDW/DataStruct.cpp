...
int List::find(long length,unsigned long long value,unsigned long long RegionIndex)//找节点，返回非零值SOrder
{
	if(first==NULL)
		return 0;
	...
	{
		p=p->link;
		if(RegionIndex==p->RegionIndex && length == p->BSLength && value == p->BSValue)
			return p->SOrder;
	}
	return 0;
}

int List::insert(long length,unsigned long long value,long SOrder,unsigned long long RegionIndex)
{
	ListNode *p = new ListNode();
	p->BSLength = length;
	p->BSValue = value;
