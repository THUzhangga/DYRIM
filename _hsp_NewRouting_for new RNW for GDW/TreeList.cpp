...
void TreeList::Add(BSCode newBSCode1, Para newPara, int SOrder)
{
	...
	if(First==NULL)//是树的根
	{
		...
		//shy, 20130903,根节点也要检查是不是支流region汇入的连接点
		try
		{
	...
	else//树已经有了根节点
	{
		...
		if(ParentNode==NULL) 
		{
			cout<<"Error while forming the whole tree on rank 0"<<endl;
			cout<<"Cannot find downstream reach of "<<newBSCode.RegionIndex<<"\t"<<newBSCode.Value<<"\t"<<newBSCode.Length<<endl;
			exit(8888666);
		}
...
bool TreeList::isCnnNode(BSCode mBSCode)
{
	...
	for(i=0;i<RCCount;i++)
	{
		if(mBSCode.RegionGrade!=RegionConnection[i].RegionGrade-1)
			continue;
		if(mBSCode.RegionIndex!=unsigned long long(RegionConnection[i].RegionIndex/MySParameter.RegionSystem))//20130904, shy
			continue;
...
void TreeList::FinishBranch(BSCode mBSCode, TreeList *TotalList, int GradeTwoCount)
{
	int i;
	//如果已经是总干流了，直接返回
	//wh，如果已经是干流，此时已经没有下游的RegionIndex了，当然不用再更新，直接返回即可
	...
	UpRegion.RegionIndex=unsigned long long(mBSCode.RegionIndex/MySParameter.RegionSystem);//20130904, shy
...
TreeNode * TreeList::FindParentPosit(BSCode mBSCode)
{
	...
	int ii;
	TreePointer=First;
	for(ii=0;ii<mBSCode.Length-First->mBSCode.Length-1;ii++)     //20130815,shy,不同Region之间衔接的BSLength不再从1开始，要减去本Region出口的BSLength
	{
		//david,好像bslength差64的还真直接走BOY。。。河网提取算法就是这样的吗，0都是偏主干的因此基本都是BOY的子代BSLENGTH可以到很高的值
		//myFile666<<mBSCode.Length-First->mBSCode.Length-1-ii<<" "<<((mBSCode.Length-First->mBSCode.Length-1-ii)>=64?0:(unsigned long long)1<<(mBSCode.Length-First->mBSCode.Length-ii-1))<<".\n";
		if(mBSCode.Value & ((mBSCode.Length-First->mBSCode.Length-1-ii)>=64?0:(unsigned long long)1<<(mBSCode.Length-First->mBSCode.Length-ii-1)))//wh:very good
