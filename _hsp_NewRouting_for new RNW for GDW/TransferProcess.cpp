...
void TransferProcess::TransferMain(void)
{
	...
	ofstream myFile777;
	myFile777.open("TransferMain.txt",ios::trunc);
	...
	while(1)
	{
		...
		{
			myFile777 <<"监测到消息10！\n"<<flush;
			//cout<<"MPI_Iprobe(MPI_ANY_SOURCE,10,MPI_COMM_WORLD,&flag,&Status)  "<<flag<<endl;
		...
		{
			myFile777 <<"监测到消息11！\n"<<flush;
			//cout<<"MPI_Iprobe(MPI_ANY_SOURCE,11,MPI_COMM_WORLD,&flag,&Status)  "<<flag<<endl;
			...
			else
			{
				...
				//不能简单地把含沙量叠加，而是要把输沙率叠加
				for(int k=0;k<Steps+1;k++)
				{
					float Qtemp;
					Qtemp=Boy->Q[k] + Girl->Q[k];
					if (Qtemp<1e-5)
					{
						temp[k]=0.0f;
					}
					...
					{
						temp[k]=(Boy->S[k]*Boy->Q[k]+Girl->S[k]*Girl->Q[k])/Qtemp;
					}
				}
				MPI_Send(temp,Steps+1,MPI_FLOAT,Status.MPI_SOURCE,11,MPI_COMM_WORLD);
		...
		{
			myFile777 <<"监测到消息12！.\n"<<flush;
			//cout<<"MPI_Iprobe(MPI_ANY_SOURCE,12,MPI_COMM_WORLD,&flag,&Status)  "<<flag<<endl;
			...
			//这里接收
			myFile777 <<"监测到消息12！1.\n"<<flush;
			MPI_Recv(&BSLength,1,MPI_LONG,Status.MPI_SOURCE,12,MPI_COMM_WORLD,&Status);
			myFile777 <<"监测到消息12！2.\n"<<flush;
			//Boy  = FindNode(RegionIndex,0,1);//bsvalue一定是0，bslength一定是1
			Boy  = FindNode(RegionIndex,0,BSLength);//bslength要变
			myFile777 <<"监测到消息12！3.\n"<<flush;
			//assert(NULL!=Boy);
			if (Boy==NULL)
			{
				myFile777 <<"监测到消息12！31.\n"<<flush;
			}
			else
			{
				myFile777 <<"监测到消息12！32.\n"<<flush;
			}
			myFile777 <<"监测到消息12！4.\n"<<flush;
			...
			cout<<"#"<<processor_name<<","<<rank<<",EXPORT,1:"<<RegionIndex<<","<<"0"<<","<<BSLength<<endl;//20130902, shy, bslength要变
			myFile777 <<"监测到消息12！9.\n"<<flush;
			GoodsOut++;
			cout<<"#"<<processor_name<<","<<rank<<",GOODSOUT,"<<GoodsOut<<":"<<endl;//20080403
			myFile777 <<"监测到消息12！10.\n"<<flush;
			capacity = (GoodsIn-GoodsOut)*size;//B
			unit = this->GetSize();//容量单位，是B，KB，MB...
			cout<<"#"<<processor_name<<","<<rank<<",CAPACITY,"<<capacity<<unit<<":"<<endl;//20080403
			myFile777 <<"监测到消息12！11.\n"<<flush;
		...
		{
			myFile777 <<"监测到消息13！\n"<<flush;
			//cout<<"MPI_Iprobe(MPI_ANY_SOURCE,13,MPI_COMM_WORLD,&flag,&Status)  "<<flag<<endl;
		...
		{
			myFile777 <<"监测到消息14！\n"<<flush;
			//cout<<"MPI_Iprobe(MPI_ANY_SOURCE,14,MPI_COMM_WORLD,&flag,&Status)  "<<flag<<endl;
