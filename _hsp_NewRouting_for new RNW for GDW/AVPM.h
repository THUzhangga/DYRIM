...
public:
    //david,以下仅对时间进行加密
	//Mysin,Q,Qout,Sout 都需要加密
	//如果储存流速、水深、河宽，FlowB，FlowH，Flow_v也要加密
	//Calc函数自不用说，需要将原来的6min步长的水沙边界条件进行时间加密，计算出口的水沙过程再还原成6min步长，得改
	//SediCalc函数，计算Sout，也需要加密的输入变量，因此也得改
	//CalFs函数所用osman冲刷公式默认取时间6min，也得改
	//注意，若考虑重力侵蚀，含水量WL和WR也得加密！

	int tjmxsh,delta_tjm; //加密多少倍
    float *Qtjm,*Qouttjm,*mySintjm,*Souttjm,*FlowBtjm,*FlowHtjm,*FlowUtjm,*WLtjm,*WRtjm;
	long iTStepstjm;  //itstepsjm=iTSteps*jmxsh
	void Calc_tjm(void); 
	void SediCalc_tjm(void);
	void CalFs_tjm(int LR);
	//void Calc_choose(void);

private:
	//记录一个河段的Qin、Sin、Qout、Sout以及SSin和SSout来看看什么导致了毛刺现象
	float SSintjm,SSouttjm,tmptjm;
	ofstream myFile2;
	float GetSSFei123(float Sv,float h,float U,float * omega);
	float GetSSYu(float Sv,float h,float U,float * omega);
