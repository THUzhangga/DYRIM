...
void HighSlopeRunoff::InitializeOneRiver(BSCode mBSCode0,Para* myPara0,float* Qu10,float* Qu20,float* Qd10,float* Qd20,float* Quout0,float* Qdout0,float* FlowB0,float* FlowH0,float* FlowV0)
{
	...
	else if(RainType.MakeLower()=="CMORPH8")//20130904, shy, add CMORPH8
	{
		XYpara=0.07277;
		PMinCMORPH.Initialize(myPara->X,myPara->Y,HourStart,NumofHours,XYpara);
		FVMunit::StartYear = FVMunit::Year = PMinCMORPH.m_YearStart;
		FVMunit::StartMonth = FVMunit::Month = PMinCMORPH.m_MonthStart;
		FVMunit::StartDay = FVMunit::Day = PMinCMORPH.m_DayStart;
		FVMunit::StartHour = FVMunit::Hour = PMinCMORPH.m_HourStart;
	}
	else if(RainType.MakeLower()=="CMORPH25")//20130904, shy, add CMORPH25
	{
		XYpara=0.25;
		PMinCMORPH.Initialize(myPara->X,myPara->Y,HourStart,NumofHours,XYpara);
		FVMunit::StartYear = FVMunit::Year = PMinCMORPH.m_YearStart;
...
void HighSlopeRunoff::Calc(BSCode mBSCode0,Para* myPara0,float* Qu10,float* Qu20,float* Qd10,float* Qd20,float* Quout0,float* Qdout0,float* FlowB0,float* FlowH0,float* FlowV0)
{
	...
	/*if(mBSCode.RegionIndex==601 && mBSCode.Length==145 && mBSCode.Value==1)
	{
		int xx=0;
	}*/
	...
	while(TimeStart<=NumofHours*3600)
	{
		...
		else if(RainType.MakeLower()=="CMORPH8" || RainType.MakeLower()=="CMORPH25")//20130904, shy, add CMORPH
		{
			StepRain=PMinCMORPH.P(double(HourStart+TimeStart/3600),&TimeNoRain,dt2);
		}
