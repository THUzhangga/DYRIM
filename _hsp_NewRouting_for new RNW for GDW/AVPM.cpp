...
void AVPM::Calc(void)
{
	...
	if(MaxQ>1.0f)
	{
		...
		if(xsteps<1) xsteps=1;
			...
			cout<<endl<<"=================================================="<<endl;
			cout<<"xsteps="<<xsteps<<" delta_t="<<delta_t<<" L="<<L<<" MaxC="<<MaxC<<" MaxQ="<<MaxQ<<endl;
			cout<<"m="<<m<<" S="<<S<<" n="<<n<<" n0="<<n0<<endl;
			cout<<"pow(m/(m*m+1),0.25f)="<<pow(m/(m*m+1),0.25f)<<endl;
			cout<<"pow(S,0.375f)="<<pow(S,0.375f)<<endl;
			cout<<"pow(MaxQ,0.25f)="<<pow(MaxQ,0.25f)<<endl;
		    cout<<endl<<"=================================================="<<endl;
	...
	for(int xx=0;xx<xsteps;xx++)//空间步长循环
	{
		...
		for(t=1;t<iTSteps;t++)//时间循环
		{
			...
			{
					float qsta;
					qsta=Q[t]/(pow(n0*Q[t]*1.5874f*pow(m*m+1.0f,0.33333333f)/pow(m,5.0f/3)/sqrt(S),0.375f)*2*m);
					n=n0*pow(qsta,0.38)*pow(S,0.32)/0.61;
			}
				//n=n0;
	...
	//tjmxsh=2;
	//delta_tjm=delta_t/tjmxsh;
	//iTStepstjm=iTSteps*tjmxsh;
	//Qtjm=new float[iTStepstjm];
	//mySintjm=new float[iTStepstjm];
	//Qouttjm=new float[iTStepstjm];
	//Souttjm=new float[iTStepstjm];
	//FlowBtjm=new float[iTStepstjm];
	//FlowHtjm=new float[iTStepstjm];
	//FlowUtjm=new float[iTStepstjm];


	////6min步长的边界条件进行加密，第一种方法：类似于“0阶重构”
	//for (int i=0;i<iTSteps;i++)
	//{
	//	for (int j=0;j<tjmxsh;j++)
	//	{
	//		Qtjm[tjmxsh*i+j]=Q[i];
	//		mySintjm[tjmxsh*i+j]=mySin[i];
	//		FlowBtjm[tjmxsh*i+j]=FlowB[i];
	//		FlowHtjm[tjmxsh*i+j]=FlowH[i];
	//		FlowUtjm[tjmxsh*i+j]=Flow_v[i];
	//		Qouttjm[tjmxsh*i+j]=Qout[i];
	//		Souttjm[tjmxsh*i+j]=Sout[i];
	//	}
	//}
	////不就是相当于在这中间加了一段代码算TJM么？？？为啥Calc_tjm报错？？
	//for (int i=0;i<iTSteps;i++)
	//{
	//	Sout[i]=Souttjm[i*tjmxsh];
	//	Qout[i]=Qouttjm[i*tjmxsh];
	//	FlowB[i]=FlowBtjm[i*tjmxsh];
	//	FlowH[i]=FlowHtjm[i*tjmxsh];
	//	Flow_v[i]=FlowUtjm[i*tjmxsh];
	//}
	//cout<<endl<<"============================="<<endl;
	//cout<<"BSlength="<<mBSCode.Length<<"  BSValue="<<mBSCode.Value<<"  t="<<t<<endl;
	//cout<<"============================="<<endl;
	//
...
float AVPM::GetSSFei(float Sv,float h,float U,float * omega)  //heli
{
	...
	float omegaS;
	omegaS=(sqrt(10.99f*myD90*myD90*myD90+36.0f*miu*miu/Rm/Rm)-6.0f*miu/Rm)/myD90;
	*omega=omegaS;
...
void AVPM::SediCalc(void)
{
	...
	//if(Qout[t]>1.1*Qout[t-1])//冲刷
	//冲淤判断条件对结果影响较明显
	//if(SSout>Sout[t-1])//冲刷, 这样太易冲刷
	//if(SSin>Sout[t-1])//冲刷,(20080828, xiaofc, 这样判断比较容易出现淤积的状态)
	//冲刷,20080828, xiaofc, 根据上游断面涨含沙量的状态直接判断是否冲刷, 同时用是否涨水加强修正, 可使冲刷的发生尽可能少
	//20090406, xiaofc补充, 这样冲刷出现的较少, 不处于涨水涨沙的状态时, 不论SS>S或SS<S, 都会取到很小的alpha, 含沙量既不会继续升高, 也不会明显下降
	//if(mySin[t]+1e-5>mySin[t-1] && Q[t]>Q[t-1])
	//if(SSin*2.0f>mySin[t]+mySin[t-1]/*&& Q[t]>Q[t-1]*/) 20090408, 怀疑因为这个原因导致在较大的洪水时, SSin>Sin在洪峰过后仍然持续，使含沙量过大
	if (SSout>Sout[t-1] && Qout[t]>0.8*Qout[t-1])/*出口挟沙力大于含沙量且在涨水*/
	{
		bIsDep=0;
		...
		tmp=NowAlphaE*omega*delta_x*(B[0]+B[1]+B[2]+B[3])/(2.0*(Q[t-1]+Qout[t-1]));//a*w*L/q
		Sout[t]=SSout+(0.95*mySin[t]+0.05*mySin[t-1]-SSin)*exp(-tmp)+(SSin-SSout)/tmp*(1.0f-exp(-tmp));
	}
	else
	{
		bIsDep=1;
		tmp=alphaD*omega*delta_x*(B[0]+B[1]+B[2]+B[3])/(Q[t-1]+Q[t]+Qout[t-1]+Qout[t]);//a*w*L/q
		//tmp=alphaD*omega*delta_x*B[2]/Q[t];//a*w*L/q
		Sout[t]=SSout+(0.2*mySin[t-1]+0.8*mySin[t]-SSin)*exp(-tmp)+(SSin-SSout)/tmp*(1.0f-exp(-tmp));
		Sout[t]=0;
	}
	...
	if(mBSCode.Length==1013 && mBSCode.RegionIndex==0 && mBSCode.Value==0) 
		t=t;
...
//david,此处仅对时间进行加密,xsteps设为1
void AVPM::Calc_tjm(void)
{
	//仅时间加密，暂不考虑空间加密steps设为1，MaxQ，MaxC计算忽略
	...
	{		
		 myFile3.open("example2.txt",ios::trunc);
        if (myFile3.is_open()) {
            myFile3 << "Cao Ping Boundary recording.\n";
			myFile3.precision(18);
        }
	...
	for(t=0;t<iTSteps;t++)
	{
		...
		if (myFile3.is_open()) 
		{
			if((t>=29668 && t<=29960) || (t>=20080 && t<=20380) || (t>=30950 && t<=31280))
			{
				myFile3 <<Qin1[t]<<" "<<Qin2[t]<<" "<<Q[t]<<" "<<SinL[t]<<" "<<SinR[t]<<" "<<mySin[t]<<"\n";
			}
			//myFile3 <<t<<" "<<Qin1[t]<<" "<<Qin2[t]<<" "<<Q[t]<<" "<<SinL[t]<<" "<<SinR[t]<<" "<<mySin[t]<<"\n";
		}
	}
	if (myFile3.is_open())
	{
	...
	//如果考虑重力侵蚀，含水量WL和WR也得加密
	if (bCalcGravity)
	{
		WLtjm=new float[iTStepstjm];
		WRtjm=new float[iTStepstjm];
	}
	...
	//		Qtjm[tjmxsh*i+j]=Q[i];
	//		mySintjm[tjmxsh*i+j]=mySin[i];
	//		FlowBtjm[tjmxsh*i+j]=0.0f;
	//		FlowHtjm[tjmxsh*i+j]=0.0f;
	//		FlowUtjm[tjmxsh*i+j]=0.0f;
	//		if (bCalcGravity)
	//		{
	//			WLtjm[tjmxsh*i+j]=WL[i];
	//			WRtjm[tjmxsh*i+j]=WR[i];
	//		}
	//	}
	//}


	//6min步长的边界条件进行加密，第二种方法：分段线性插值
	for (int i=0;i<iTSteps;i++)
	{
		for (int j=0;j<tjmxsh;j++)
		{
			if (i<iTSteps-1)
			{
				Qtjm[tjmxsh*i+j]=Q[i]+(Q[i+1]-Q[i])/tjmxsh*j;
				mySintjm[tjmxsh*i+j]=mySin[i]+(mySin[i+1]-mySin[i])/tjmxsh*j;
			}
			else
			{
				Qtjm[tjmxsh*i+j]=Q[i];
	...
	//输出曹坪站所在河段入口、出口流量和含沙量过程，首先需要判定河段是不是曹坪
	if (mBSCode.RegionIndex==0 && mBSCode.Length==4 && mBSCode.Value==0)
	//if (mBSCode.Length==67 && mBSCode.Value==65536)
	{		
		 myFile2.open("example.txt",ios::trunc);
        if (myFile2.is_open()) {
            myFile2 << "Cao Ping recording.\n";
			myFile2.precision(18);
        }
	...
	for(int xx=0;xx<xsteps;xx++)//空间步长循环
	{
		...
		if(xx)//把非第0空间层新的Qout给Q
		{
			for(t=0;t<iTStepstjm;t++)
				Qtjm[t]=Qouttjm[t];
		}
		else
			GravityErosion[0]=0;

		Qouttjm[0]=Qtjm[0];//第一个时间点不进行计算
		mySintjm[0]=0.0f;
		Souttjm[0]=0.0f;
		...
		//if(Q[0]<1 && n0<0.5)
		//	n=0.5-(0.5-n0)*Q[0];
		if(Qtjm[0]<mPara.DrainingArea/1000000000*1 && n0<0.5  && mPara.DrainingArea>3000000000)
			n=0.5-(0.5-n0)*Qtjm[0]/1*1000000000/mPara.DrainingArea;
		else
			//20180317阻力分解
		{
				float qsta;
				qsta=(Qtjm[0]+Qouttjm[0])*0.5/(pow(n0*(Qtjm[0]+Qouttjm[0])*0.5*1.5874f*pow(m*m+1.0f,0.33333333f)/pow(m,5.0f/3)/sqrt(S),0.375f)*2*m);
				n=n0*pow(qsta,0.38)*pow(S,0.32)/0.61;
		}
			//n=n0;

		h[1]=pow(n*Qtjm[0]*1.5874f*pow(m*m+1.0f,0.33333333f)/pow(m,5.0f/3)/sqrt(S),0.375f);
		h[3]=pow(n*Qouttjm[0]*1.5874f*pow(m*m+1.0f,0.33333333f)/pow(m,5.0f/3)/sqrt(S),0.375f);
		...
		for(t=1;t<iTStepstjm;t++)//时间循环
		{
			...
			//20080306,xiaofc,重力侵蚀入水量状态的初始化
			//必须放在前面，因为有些时间循环是不计算直接continue的
			DayLoop=t*MSTEP/60/24/tjmxsh;
			if ( (t*MSTEP) % (60*24*tjmxsh)==0 && xx==0)
				GravityErosion[DayLoop]=0.0f;
			...
			//if(Q[t]<1 && n0<0.5)
			//	n=0.5-(0.5-n0)*Q[t];
			if(Qtjm[t]<mPara.DrainingArea/1000000000*1 && n0<0.5  && mPara.DrainingArea>3000000000)
				n=0.5-(0.5-n0)*Qtjm[t]/1*1000000000/mPara.DrainingArea;
			else
			{
				float qsta;
				qsta=Qtjm[t]/(pow(n0*Qtjm[t]*1.5874f*pow(m*m+1.0f,0.33333333f)/pow(m,5.0f/3)/sqrt(S),0.375f)*2*m);
				n=n0*pow(qsta,0.38)*pow(S,0.32)/0.61;
			}
				//n=n0;
			...
			//cout<<"Time Step "<<t<<endl;
			if(isDebug)
				myFile<<"Time Step "<<t<<"\tQ0="<<Qtjm[t]<<endl;

			//20060310,李铁键,如果Q[t]<1e-5,那么直接下一时间循环
			if(Qtjm[t]<1e-5)
			{
				Qouttjm[t]=Qtjm[t];//0.0f;
				Souttjm[t]=mySintjm[t];//0.0f;
				if (myFile2.is_open())
				{
					if((t>=29668*tjmxsh && t<=29960*tjmxsh) || (t>=20080*tjmxsh && t<=20380*tjmxsh) || (t>=30950*tjmxsh && t<=31280*tjmxsh))
					{
						myFile2<<t<<" "<<m<<" "<<n<<" "<<J<<" "<<S<<" "<<h[0]<<" "<<h[1]<<" "<<h[2]<<" "<<h[3]<<" "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" "<<u[3]<<" "<<Qtjm[t]<<" "<<mySintjm[t]<<" "<<SSintjm<<" "<<Qouttjm[t]<<" "<<Souttjm[t]<<" "<<SSouttjm<<" "<<tmptjm<<" c1"<<"\n";
					}
					//myFile2<<t<<" "<<m<<" "<<n<<" "<<J<<" "<<S<<" "<<h[0]<<" "<<h[1]<<" "<<h[2]<<" "<<h[3]<<" "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" "<<u[3]<<" "<<Qtjm[t]<<" "<<mySintjm[t]<<" "<<SSintjm<<" "<<Qouttjm[t]<<" "<<Souttjm[t]<<" "<<SSouttjm<<" "<<tmptjm<<" c1"<<"\n";
				}
				continue;
			...
			//Qout[t]=Qout[t-1];
			Qouttjm[t]=Qtjm[t];//用Qt比Qout t-1 更容易收敛些


			h[0]=h[1];
			h[2]=h[3];

			h[1]=pow(n*Qtjm[t]*1.5874f*pow(m*m+1.0f,0.33333333f)/pow(m,5.0f/3)/sqrt(S),0.375f);
			h[3]=pow(n*Qouttjm[t]*1.5874f*pow(m*m+1.0f,0.33333333f)/pow(m,5.0f/3)/sqrt(S),0.375f);


			//求pian h / pian x
			hh=(h[0]+h[1]+h[2]+h[3])/4.0f;

			u[0]=(Qtjm[t-1])/h[0]/h[0]/m;
			u[1]=(Qtjm[t])/h[1]/h[1]/m;
			u[2]=(Qouttjm[t-1])/h[2]/h[2]/m;
			u[3]=(Qouttjm[t])/h[3]/h[3]/m;
			c[0]=u[0]*4.0/3.0;
			c[1]=u[1]*4.0/3.0;
			c[2]=u[2]*4.0/3.0;
			c[3]=u[3]*4.0/3.0;
			...
			BAverage=4.0f*m*(Qtjm[t-1]+Qtjm[t]+Qouttjm[t-1]+Qouttjm[t])/3.0f/cAverage;
			BAverage=sqrt(BAverage);
			...
			//20080811, xiaofc, 计算ph/px不采用Qout[t]点的值，那样对收敛不利
			//Q_x=(Q[t-1]+Q[t]-Qout[t-1]-Qout[t])/2.0f/delta_x;
			Q_x=((Qtjm[t-1]+Qtjm[t])/2.0f-Qouttjm[t-1])/delta_x;
			h_x=Q_x/BAverage/cAverage;
			...
			C1=(K*iksila+0.5*delta_tjm)/(K*(1.0f-iksila)+0.5*delta_tjm);
			C2=(-K*iksila+0.5*delta_tjm)/(K*(1.0f-iksila)+0.5*delta_tjm);
			C3=(K*(1.0f-iksila)-0.5*delta_tjm)/(K*(1.0f-iksila)+0.5*delta_tjm);

			//新的Qout[t]
			iterQ=C1*Qtjm[t-1]+C2*Qtjm[t]+C3*Qouttjm[t-1];
			...
			if(iterQ<1e-10)
			{
				Qouttjm[t]=1e-10;//(Q[t]+Qout[t-1]/2.0f);//0.0f;b
				//Qouttjm[t]=(Qtjm[t]+Qouttjm[t-1])/2.0f;
				Souttjm[t]=(mySintjm[t]+Souttjm[t-1])/2.0f;//0.0f;
				if(SaveFlowPattern)
				{
					FlowBtjm[t]=FlowBtjm[t-1];
					FlowHtjm[t]=FlowHtjm[t-1];
					FlowUtjm[t]=FlowUtjm[t-1];
				}
				if (myFile2.is_open())
				{
					if((t>=29668*tjmxsh && t<=29960*tjmxsh) || (t>=20080*tjmxsh && t<=20380*tjmxsh) || (t>=30950*tjmxsh && t<=31280*tjmxsh))
					{
						myFile2<<t<<" "<<m<<" "<<n<<" "<<J<<" "<<S<<" "<<h[0]<<" "<<h[1]<<" "<<h[2]<<" "<<h[3]<<" "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" "<<u[3]<<" "<<Qtjm[t]<<" "<<mySintjm[t]<<" "<<SSintjm<<" "<<Qouttjm[t]<<" "<<Souttjm[t]<<" "<<SSouttjm<<" "<<tmptjm<<" c2"<<"\n";
					}
					//myFile2<<t<<" "<<m<<" "<<n<<" "<<J<<" "<<S<<" "<<h[0]<<" "<<h[1]<<" "<<h[2]<<" "<<h[3]<<" "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" "<<u[3]<<" "<<Qtjm[t]<<" "<<mySintjm[t]<<" "<<SSintjm<<" "<<Qouttjm[t]<<" "<<Souttjm[t]<<" "<<SSouttjm<<" "<<tmptjm<<" c2"<<"\n";
				}
				continue;
			...
			IterCount=0;
			while( !(abs(iterQ-Qouttjm[t])<0.1 || abs(iterQ/Qouttjm[t]-1.0f)<0.005 || IterCount>MaxIter))//依上面的方法迭代，至Qout[t]收敛
			{
				IterCount++;
				...
				C1=(K*iksila+0.5*delta_tjm)/(K*(1.0f-iksila)+0.5*delta_tjm);//小
				C2=(-K*iksila+0.5*delta_tjm)/(K*(1.0f-iksila)+0.5*delta_tjm);
				C3=(K*(1.0f-iksila)-0.5*delta_tjm)/(K*(1.0f-iksila)+0.5*delta_tjm);
				...
				if(iterQ<1e-10)
				{
					Qouttjm[t]=1e-10;//(Q[t]+Qout[t-1]/2.0f);//0.0f;
					//Qouttjm[t]=(Qtjm[t]+Qouttjm[t-1])/2.0f;
					Souttjm[t]=(mySintjm[t]+Souttjm[t-1])/2.0f;//0.0f;
					if(SaveFlowPattern)
					{
			...
			if(bNegtiveQHappened)
			{
				if (myFile2.is_open())
				{
					if((t>=29668*tjmxsh && t<=29960*tjmxsh) || (t>=20080*tjmxsh && t<=20380*tjmxsh) || (t>=30950*tjmxsh && t<=31280*tjmxsh))
					{
						myFile2<<t<<" "<<m<<" "<<n<<" "<<J<<" "<<S<<" "<<h[0]<<" "<<h[1]<<" "<<h[2]<<" "<<h[3]<<" "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" "<<u[3]<<" "<<Qtjm[t]<<" "<<mySintjm[t]<<" "<<SSintjm<<" "<<Qouttjm[t]<<" "<<Souttjm[t]<<" "<<SSouttjm<<" "<<tmptjm<<" c3"<<"\n";
					}
					//myFile2<<t<<" "<<m<<" "<<n<<" "<<J<<" "<<S<<" "<<h[0]<<" "<<h[1]<<" "<<h[2]<<" "<<h[3]<<" "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" "<<u[3]<<" "<<Qtjm[t]<<" "<<mySintjm[t]<<" "<<SSintjm<<" "<<Qouttjm[t]<<" "<<Souttjm[t]<<" "<<SSouttjm<<" "<<tmptjm<<" c3"<<"\n";
				}
				continue;
			...
			{
				FlowBtjm[t]=BAverage;
				FlowHtjm[t]=(h[0]+h[1]+h[2]+h[3])/4.0f;
				FlowUtjm[t]=cAverage*0.75f;
			}
			...
			SediCalc_tjm();
				if (myFile2.is_open())
				{
					if((t>=29668*tjmxsh && t<=29960*tjmxsh) || (t>=20080*tjmxsh && t<=20380*tjmxsh) || (t>=30950*tjmxsh && t<=31280*tjmxsh))
					{
						myFile2<<t<<" "<<m<<" "<<n<<" "<<J<<" "<<S<<" "<<h[0]<<" "<<h[1]<<" "<<h[2]<<" "<<h[3]<<" "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" "<<u[3]<<" "<<Qtjm[t]<<" "<<mySintjm[t]<<" "<<SSintjm<<" "<<Qouttjm[t]<<" "<<Souttjm[t]<<" "<<SSouttjm<<" "<<tmptjm<<" e"<<"\n";
					}
					//myFile2<<t<<" "<<m<<" "<<n<<" "<<J<<" "<<S<<" "<<h[0]<<" "<<h[1]<<" "<<h[2]<<" "<<h[3]<<" "<<u[0]<<" "<<u[1]<<" "<<u[2]<<" "<<u[3]<<" "<<Qtjm[t]<<" "<<mySintjm[t]<<" "<<SSintjm<<" "<<Qouttjm[t]<<" "<<Souttjm[t]<<" "<<SSouttjm<<" "<<tmptjm<<" e"<<"\n";
				}
		}//end of timeloop
	...
	//此处再把计算出的加密的Qout和Sout转换成6min步长，同时，删掉原有的数组释放内存空间
	//试一下jmxsh=1的情况下计算结果与原来是否一致
	for (int i=0;i<iTSteps;i++)
	{
		Sout[i]=Souttjm[i*tjmxsh];
		Qout[i]=Qouttjm[i*tjmxsh];
	...
	for(t=0;t<iTSteps;t++)
	{
		...
		if(Qout[t]<1e-5)
			...
			Sout[t]=(aaaaa*tmp+SinMe[t]*Qin3[t])/Qout[t];
		...
		if (myFile2.is_open())
		{
			if((t>=29668 && t<=29960) || (t>=20080 && t<=20380) || (t>=30950 && t<=31280))
			{
				myFile2<<t<<" "<<tmp<<" "<<aaaaa<<" "<<Qin3[t]<<" "<<SinMe[t]<<" "<<Qout[t]<<" "<<Sout[t]<<" Self"<<"\n";
			}
		}
		if(!_finite (Sout[t]))
		{
...
void AVPM::CalFs_tjm(int LR)   //LR 0:左，1:右
{
	switch(LR){
	case 0:{
		Height=HeightL;
	...
	if(Tao>Tao_c)
	{
		...
		Delt_B=3.64e-4*(Tao-Tao_c)*exp(-1.3f*Tao_c)*6.0f/tjmxsh;  //计算横向冲刷宽度,后边的6是6分钟时间间隔
...
void AVPM::SediCalc_tjm(void)
{
	...
	if((Qtjm[t-1]+Qtjm[t])<1e-5 || (Qtjm[t-1]+Qtjm[t]+Qouttjm[t-1]+Qouttjm[t])<1e-5)
	{
		Souttjm[t]=0.0f;
		return;
	}
	...
	if(SSMethod=="Zhang")
	{
		SSin=GetSS((mySintjm[t-1]+mySintjm[t])/2.0/gammaS,(h[0]+h[1])/2.0,(u[0]+u[1])/2.0,&tmp);
		SSout=GetSS(Souttjm[t-1]/gammaS,h[2],u[2],&omega);
	}
	else if(SSMethod=="Fei")
	{	
		SSin=GetSSFei((mySintjm[t-1]+mySintjm[t])/2.0/gammaS,(h[0]+h[1])/2.0,(u[0]+u[1])/2.0,&tmp);  //heli
		SSout=GetSSFei(Souttjm[t-1]/gammaS,h[2],u[2],&omega);
	}
	else if(SSMethod=="Yu")
	{	
		SSin=GetSSYu((mySintjm[t-1]+mySintjm[t])/2.0/gammaS,(h[0]+h[1])/2.0,(u[0]+u[1])/2.0,&tmp);  //heli
		//SSin=GetSSYu(mySintjm[t-1]/gammaS,h[0],u[0],&tmp);
		SSout=GetSSYu(Souttjm[t-1]/gammaS,h[2],u[2],&omega);
	}
	else
		cout<<"Error of SSMethod"<<endl;
	...
	//if(mySin[t]+1e-5>mySin[t-1] && Q[t]>Q[t-1])
	//if(SSin*2.0f>mySin[t]+mySin[t-1]/*&& Q[t]>Q[t-1]*/) 20090408, 怀疑因为这个原因导致在较大的洪水时, SSin>Sin在洪峰过后仍然持续，使含沙量过大
	//if (SSout>Souttjm[t-1] && Qouttjm[t]>0.8*Qouttjm[t-1])/*出口挟沙力大于含沙量且在涨水*/
	if (SSout>Souttjm[t-1])
	{
		bIsDep=0;
		//tmp=alphaE*omega*delta_x*(B[0]+B[1]+B[2]+B[3])/(Q[t-1]+Q[t]+Qout[t-1]+Qout[t]);//a*w*L/q
		//tmp=NowAlphaE*omega*delta_x*(B[0]+B[1]+B[2]+B[3])/(Q[t-1]+Q[t]+Qout[t-1]+Qout[t]);//a*w*L/q
		tmp=NowAlphaE*omega*delta_x*(B[0]+B[1]+B[2]+B[3])/(2.0*(Qtjm[t-1]+Qouttjm[t-1]));//a*w*L/q
		tmptjm=tmp;
		Souttjm[t]=SSout+(0.95*mySintjm[t]+0.05*mySintjm[t-1]-SSin)*exp(-tmp)+(SSin-SSout)/tmp*(1.0f-exp(-tmp));

		/*Souttjm[t]=795.0f+(0.95*mySintjm[t]+0.05*mySintjm[t-1]-795)*exp(-tmp);
		Souttjm[t]=795.0f+(mySintjm[t-1]-795)*exp(-tmp);
		Souttjm[t]=795.0f+(mySintjm[t-1]-795)*exp(-1);*/
		//尝试改变一下
		//tmp=NowAlphaE*omega*delta_x*(B[0]+B[1]+B[2]+B[3])/(2.0*(Qtjm[t-1]+Qouttjm[t-1]));
		//tmptjm=tmp;
		//Souttjm[t]=SSout+(0.95*mySintjm[t]+0.05*mySintjm[t-1]-SSin)*exp(-tmp)+(SSin-SSout)/tmp*(1.0f-exp(-tmp));

		//if (Souttjm[t]>795.0f && mySintjm[t-1]<=795.0f)
		//{
		//	cout<<endl<<"===================================================="<<endl;
		//	cout<<"===================================================="<<endl;
		//	cout<<"===================================================="<<endl;
		//	cout<<"Souttjm="<<Souttjm[t]<<"mySintjm[t-1]="<<mySintjm[t-1]<<" tmp="<<tmp<<" exp(-tmp)="<<exp(-tmp)<<endl;
		//	cout<<"===================================================="<<endl;
		//	cout<<"===================================================="<<endl;
		//	cout<<"===================================================="<<endl;
		//}
	...
	{
		bIsDep=1;
		tmp=alphaD*omega*delta_x*(B[0]+B[1]+B[2]+B[3])/(Qtjm[t-1]+Qtjm[t]+Qouttjm[t-1]+Qouttjm[t]);//a*w*L/q
		//tmp=alphaD*omega*delta_x*B[2]/Q[t];//a*w*L/q
		tmptjm=tmp;
		Souttjm[t]=SSout+(0.2*mySintjm[t-1]+0.8*mySintjm[t]-SSin)*exp(-tmp)+(SSin-SSout)/tmp*(1.0f-exp(-tmp));
		/*Souttjm[t]=795+(0.2*mySintjm[t-1]+0.8*mySintjm[t]-795)*exp(-tmp);
		Souttjm[t]=795+(mySintjm[t-1]-795)*exp(-tmp);
		Souttjm[t]=795.0f+(mySintjm[t-1]-795)*exp(-1);*/

		//尝试改变一下
		//tmp=alphaD*omega*delta_x*(B[0]+B[1]+B[2]+B[3])/(Qtjm[t-1]+Qtjm[t]+Qouttjm[t-1]+Qouttjm[t]);
		//tmptjm=tmp;
		//Souttjm[t]=SSout+(0.2*mySintjm[t-1]+0.8*mySintjm[t]-SSin)*exp(-tmp)+(SSin-SSout)/tmp*(1.0f-exp(-tmp));
	...
	//20051217,李铁键：增加含沙量异常时的调试信息
	if(isDebug)
		myFile<<"t="<<t<<"\tSout[t]="<<Souttjm[t]<<"\tSSout="<<SSout<<"\tSSin="<<SSin<<"\tmySin[t-1]="<<mySintjm[t-1]<<"\tmySin[t]="<<mySintjm[t]<<"\ttmp="<<tmp<<endl;

	if(Souttjm[t]<0.0001f)
		Souttjm[t]=0.0001f;
	else if(Souttjm[t]>gammaS*Svm)
	Souttjm[t]=gammaS*Svm-1.0f;
	...
	if (bCalcGravity &&SlideAmount>0&&bIsDep==0)
	{
		...
		DeltaSlideAmount=(Souttjm[t]*Qouttjm[t]-Qtjm[t]*mySintjm[t])*delta_tjm;
		...
		if(DeltaSlideAmount>SlideAmount)
		{
			Souttjm[t]-=(DeltaSlideAmount-SlideAmount)/delta_tjm/Qouttjm[t];
			DeltaSlideAmount=SlideAmount;
		}
...
float AVPM::GetSSFei123(float Sv,float h,float U,float * omega)  //heli
{
	float myD90=D90;
	//if(U<3.5f)
	//	myD90=D90*3.5f/U;
	...
	//20070724,xiaofc,采用费祥俊的公式计算浑水沉速
	float omegaS,omegaSS;
	omegaS=(sqrt(10.99f*myD90*myD90*myD90+36.0f*miu*miu/Rm/Rm)-6.0f*miu/Rm)/myD90/3.5;
	omegaSS=(sqrt(10.99f*D50*D50*D50+36.0f*miu*miu/Rm/Rm)-6.0f*miu/Rm)/D50/3.5;
	*omega=omegaSS;

	float Re=4.0*RR*U*Rm/miu;
...
float AVPM::GetSSYu(float Sv,float h,float U,float * omega)
{
	float myD90=D90;
	float miu=0.001;
	float miuv=0.001/1000;
	float Rm=1000;
	float omegaSS,dstar;
	dstar=pow(1.65*9.8/miuv/miuv,1.0f/3.0f)*D50;
	//omegaSS=pow(sqrt(25.0f+1.2*dstar*dstar)-5.0f,1.5)*miuv/D50;
	omegaSS=(sqrt(10.36*10.36+1.049*pow(1-0,4.7)*dstar*dstar*dstar)-10.36)*miuv/D50;
	//omegaSS=(sqrt(10.36*10.36+1.049*pow(1-Sv,4.7)*dstar*dstar*dstar)-10.36)*miuv/D50;
	*omega=omegaSS;
	float Lambda;
	//三角形断面，R不再等于h
	//Lambda=1.0f/1.65*U*S/omegaSS*m/sqrt(m*m+1.0f)/2.0f-1.5;
	//考虑临界启动对应的无量纲化水流功率，暂定为0.1
	Lambda=1.0f/1.65*U*S/omegaSS-3.5;
	//Lambda=1.0f/1.65*U*S/omegaSS-1.5;
	if (Lambda<0.0f)
		Lambda=0.0f;
	float SSYu;
	SSYu=0.338*(1-exp(-0.115*Lambda));
	//SSYu=0.434*(1-exp(-0.115*Lambda));
	return SSYu*2650.0f;
}