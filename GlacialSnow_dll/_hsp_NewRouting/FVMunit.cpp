...
void FVMunit::UnsaturatedSoil(float dt)
{
	...
	for(int i=0;i<N;i++)
	{
		...
		if(HighSlopeRunoff::debug && ri[i].theta>thetas+1e-1)
		{
			if(flag==false)
			{
				flag=true;
			...
			{
				(*myFile)<<"当前土壤表层入渗:"<<ufe*1000<<"(mm/s),网格厚"<<ri[i].dz/1000<<"(m),土壤含水量:"<<ri[i].theta<<",大于"<<thetas<<endl;
			}
		}

		if(HighSlopeRunoff::debug && ri[i].theta<-1e-2)
		{
			if(flag==false)
			{
				flag=true;
			...
			{
				(*myFile)<<"当前土壤表层入渗:"<<ufe*1000<<"(mm/s),网格厚"<<ri[i].dz/1000<<"(m),土壤含水量:"<<ri[i].theta<<",小于0"<<endl;
			}
		}
