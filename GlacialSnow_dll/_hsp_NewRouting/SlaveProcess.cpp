...
//李铁键产流产沙模型dll组件调用
#include "C:\Hydro_Cool\Graduation\SoftWare\src 1.0\WaterYield_dll\WaterYield\WaterYield.h"
#include "C:\Hydro_Cool\Graduation\SoftWare\src 1.0\WaterYield_dll\WaterYield\WaterYield_i.c"
#include "C:\Hydro_Cool\Graduation\SoftWare\src 1.0\XAJ_dll\新安江模型\新安江模型\My.h"
#include "C:\Hydro_Cool\Graduation\SoftWare\src 1.0\XAJ_dll\新安江模型\新安江模型\My_i.c"
...
void SlaveProcess::SlaveProcessInitialize(void)
{
	...
	//读取全流域面积，分配基流
	float BasinArea;
	CSSQL.Format("Select catchmentarea from riversegs where regionindex=0 and bsvalue=0 and bslength=1");
	bSQL = CSSQL.GetString();
	try
	{
		pRstIndicator->Open(bSQL,(ADODB::_Connection*)pCnn,ADODB::adOpenStatic,ADODB::adLockOptimistic,ADODB::adCmdText);
	...
	{
		cout<<"Failture Of Reading CatchmentArea of RegionIndex=0 and bsvalue=0 and bslength=1."<<rank<<endl;
	...
	else//不计算水库以上的汇流和水库过程
	{
		...
		try
		{
			...
			while (!pRstIndicator->EndOfFile) 
			{
				...
				RsvList->insert(pRstIndicator->Fields->Item["BSLength"]->Value,vtmp.ullVal,-1);//水库没有Sorder，所以写为-1
				pRstIndicator->MoveNext();
			}
			pRstIndicator->Close();
		}
...
void SlaveProcess::RecvMasterProcess(void)
{
	...
	while(1)
	{
		...
		for(TaskLoop=0; TaskLoop<TaskCount; TaskLoop++)
		{
			...
			//如果是水库节点
			if(RsvList->find(mBSCode.Length,mBSCode.Value))
			{
			...
			else
			{
				...
				if(mTreeNode[TaskLoop].StralherOrder>1)
				{
					...
					{			
						if((mTreeNode[j].mBSCode.Length==mBSCode.Length+1) && (mTreeNode[j].mBSCode.Value==mBSCode.Value<<1)) 
						{
							Boy=j;
						}
						else if((mTreeNode[j].mBSCode.Length==mBSCode.Length+1) && (mTreeNode[j].mBSCode.Value==(mBSCode.Value<<1)+1))
						{ 
							Girl=j;
						}		
					}
...
void SlaveProcess::ReservoirSegment(BSCode mBSCode,int TaskLoop)
{
	...
	float preS,nowS;
	long preHourOffset,nowHourOffset;
	float calHourOffset;
	CString cSQL;
	_bstr_t bSQL;
	CComVariant tempVar;
	try
	{
		...
		cSQL.Format("Select * from reservoir where BSValue=%I64u and BSLength=%d and houroffset>=%d and houroffset<%d and regionindex=%I64u order by houroffset",\
			         mBSCode.Value,mBSCode.Length,SParameter.HourStart,SParameter.HourStart+SParameter.NumofHours,mBSCode.RegionIndex);
		bSQL = cSQL.GetString();
		pRstRsv->Open(bSQL,(ADODB::_Connection*)pCnn,ADODB::adOpenStatic,ADODB::adLockOptimistic,ADODB::adCmdText);
		if(pRstRsv->EndOfFile) { cout<<"No reservoir data for "<<mBSCode.Length<<"----"<<mBSCode.Value<<endl;}	
		pRstRsv->MoveFirst();
		...
		{
			calHourOffset = SParameter.HourStart+j*lTimeInterval/3600.0f;
...
void SlaveProcess::ConfluenceCalc(int TaskLoop,BSCode mBSCode,Para mPara)
{
	...
	else //算汇流
	{
		...
		else if((BasinModel[ModelSelect][1].MakeLower() == "avpm") || (BasinModel[ModelSelect][1].MakeLower() == "none"))//精确扩散波法//wh
		{
			try
			{
...
void SlaveProcess::NoChild(BSCode mBSCode)
{
	...
	{
		MPI_Send(&RegionConnection[i].RegionIndex,1,MPI_UNSIGNED_LONG_LONG,TransferProcessRank,12,MPI_COMM_WORLD);//bsvalue一定是0，bslength一定是1，所以不用发送
...
void SlaveProcess::WriteToDischarge(int TaskLoop)
{
	...
	try
	{
		cSQL.Format("Select * from DefinedNodes where RegionIndex=%I64u and BSValue=%I64u and BSLength=%d and nodetype in ('CS','GS')",mTreeNode[TaskLoop].mBSCode.RegionIndex,mTreeNode[TaskLoop].mBSCode.Value,mTreeNode[TaskLoop].mBSCode.Length);
		SQL=cSQL.GetString();
		...
		else//写入
		{
			...
			if(bIsRewrite)//改写
			{
				...
				comSQL=cSQL;
				pRstData->Open(comSQL.bstrVal,(ADODB::_Connection*)pCnn,ADODB::adOpenForwardOnly,ADODB::adLockBatchOptimistic,ADODB::adCmdText);
				...
				if(BasinModel[ModelSelect][0].MakeLower()=="ltjyr")
				{
					for(j=lNoStorageHours*60/SParameter.MSTEP;j<Steps;j++)
					{
						...
						else{ pRstData->Fields->Item["SOutput"]->Value=0;}
			...
			else//插入
			{
				...
				if(BasinModel[ModelSelect][0].MakeLower() == "ltjyr")
				{
					for(j=lNoStorageHours*60/SParameter.MSTEP;j<Steps;j++)
					{
						...
						if(SParameter.iCalcSediTrans) {pRstData->Fields->Item["SOutput"]->Value=pSout[Parent][j];}
						else { pRstData->Fields->Item["SOutput"]->Value=0;}
