...
void Cyuliang::Initialize(double x,double y,long StartHour, long Hours)
{
	...
	DayCount=long(StartHour/24); //转换小时为天//计算开始时间距离基准年的天数
    temp=0;
...
//wh，该函数实现了将介于计算起止时间之内每一天的降雨量组织成链表。
PSeriesType Cyuliang::GetGageTS(long ID, short YearStart,short MonthStart, short DayStart,short YearEnd, short MonthEnd, short DayEnd)
{
	...
	//wh，将雨量表中YearStart,MonthStart行记录的DayStart到本月最后一天的雨量数据组织成链表
...
//wh，得到计算起止时间内，河段的降雨序列。降雨序列的每一项为一天，包括DayStart和DayEnd。
...
//得到分摊到步长时间内的降雨量
float Cyuliang::P(double HourOffset)
{
	...
	{
		i=m_PSeries.size()-1;
		tempflt=m_PSeries[i]/1000.0;
	}
	else
	{
		tempflt=m_PSeries[i]/1000.0;//wh，这里除以1000，肯定是转化成m的意思吧。
	}
	//cout<<"HourOffset-StartTime="<<(HourOffset-StartTime)<<"tempflt="<<tempflt<<endl;
	Result=ScatterDayToHours((HourOffset-StartTime),tempflt);
	
	//上面都除以1000了，肯定已经转化成m了。
	//20060109,李铁键,确保返回的是时间点附近，MSTEP分钟数内降雨量的mm数Result/MSTEP->Result*MSTEP/60
	return Result*MSTEP/60;
}
...
float Cyuliang::ScatterDayToHours(long HourOffset,float rainfall)
{
	...
	//wh：到了新的一天，一下子就全分配好了，所以NeedReset经常为false
	if(NeedReset){
		IndexVec.clear();
		ValueVec.clear();
