...
应用程序向导已为您创建了此 新安江模型 项目，您可以在此基础之上
编写自己的动态链接库 (DLL)。
...
My.idl
    ...
    和 coclass 的 IDL 定义。
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明                 (My.h)
        GUID 定义                                (My_i.c)
        一个类型库                               (My.tlb)
        封送代码                                 (My_p.c 和 dlldata.c)

My.h
    此文件包含在 My.idl 中定义的项的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 新安江模型.pch 的预编译头 (PCH) 文件
